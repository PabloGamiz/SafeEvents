...
// HandleCheckRequest attends a signin request
func HandleCheckRequest(w http.ResponseWriter, r *http.Request) {
	log.Printf("Handlering a Check request")

	// Expected data for a purchase request
	var checkDTO ticketDTO.CheckRequestDTO
	...
	txPurchase := ticket.NewTxCheck(checkDTO)
